...
void score() {
  char student_answers[10]; //Array para guardar las respuestas del usuario.
  char real_score[] = {'A', 'C', 'B', 'D', 'A', 'B', 'C', 'D', 'A', 'B'}; //Respuestas correctas del examen
  int correct = 0;
...
void discount() {
  ...
  while ((getchar()) != '\n'); //Elimina el nuevo caracter de linea del printf anterior.

  printf("El producto es electronico (S/N):");
  scanf("%c", &is_electronic);
  is_electronic = toupper(is_electronic); //Parsea la letra a mayuscula.
